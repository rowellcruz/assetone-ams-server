...
async function getDepartmentByID(id) {
  const { rows } = await db.query("SELECT * FROM departments WHERE id = $1", [
    id,
  ]);
  return rows[0];
}
...
async function updateDepartmentPartial(id, fields) {
  ...
  const { rowCount } = await db.query(
    `UPDATE departments SET ${updates} WHERE id = $${keys.length + 1}`,
    [...values, id]
  );
...
async function deleteDepartmentByID(id) {
  const { rowCount } = await db.query("DELETE FROM departments WHERE id = $1", [
    id,
  ]);
  return rowCount > 0;
}
