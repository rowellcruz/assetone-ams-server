...
async function getAllItemDepreciation() {
  const { rows } = await db.query("SELECT * FROM item_depreciation");
  return rows;
}

async function getItemDepreciationByID(id) {
  const { rows } = await db.query(
    "SELECT * FROM item_depreciation WHERE id = $1",
    [id]
  );
  return rows[0];
}
...
async function deleteItemDepreciationByIDs(ids) {
  ...
  await db.query(
    `DELETE FROM item_depreciation WHERE id IN (${placeholders})`,
    ids
  );
}
...
async function updateItemDepreciationPartial(id, fields) {
  ...
  const { rowCount } = await db.query(
    `UPDATE item_depreciation SET ${updates} WHERE id = $${keys.length + 1}`,
    [...values, id]
  );
...
async function deleteItemDepreciationByID(id) {
  const { rowCount } = await db.query(
    "DELETE FROM item_depreciation WHERE id = $1",
    [id]
  );
  return rowCount > 0;
}
