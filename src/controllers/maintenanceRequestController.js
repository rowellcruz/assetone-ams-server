...
export const getMaintenanceRequests = async (req, res) => {
  ...
  const reports = await maintenanceRequestsService.getMaintenanceRequests(filters);
  res.status(201).json(reports);
};

export const getMaintenanceRequestsByItemUnitId = async (req, res) => {
  const { id, status } = req.params;
  const reports = await maintenanceRequestsService.getMaintenanceRequestsByItemUnitId(id, status);
  res.status(201).json(reports);
};
...
export const handleReportApproval = async (req, res) => {
  ...
  const createdRequest = await maintenanceRequestsService.approveIssueReport(reportData);
  res.status(201).json(createdRequest);
};