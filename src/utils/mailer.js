...
export async function sendRegistrationRejection(email, firstName) {
  const html = `
    ...
    <p>Best regards,<br>AssetONE Team</p>
  `;
  return sendEmail(email, "Registration Not Approved - Asset Management System", html);
}
...
export async function sendResetConfirmation(email, rawToken) {
  ...
  const html = `
    ...
    <p>Best regards,<br>AssetONE Team</p>
  `;
  return sendEmail(email, "Password Reset Request - Asset Management System", html);
}
