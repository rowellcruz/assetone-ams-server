  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setPlannedCost(prRes.planned_cost);
      } catch (err) {
    ...
    <div className="text-gray-700">
      ...
      <div className="border p-6 rounded-xl bg-gray-50 mb-6">
        <label className="text-sm text-gray-600 mb-1 block">
          Approved Budget (Planned Cost)
        </label>
        {purchaseRequest.status !== "pending" ? (
          <p className="text-gray-500 italic bg-gray-100 px-3 py-2 rounded-lg">
            {plannedCost
              ? `₱${Number(plannedCost).toLocaleString("en-PH", {
